            ...
            if isinstance(prompt_config, list):
                ...
                return {
                    name: {
                        ...
                        'tonality_messages': tonality_msgs,
        ...
        try:
            for config in extraction_configs:
                ...
                if context_data is None:
                    logger.warning(f"⚠️ Context data is missing for extraction: {subsection_name}")
                    results[name] = {
                        'json_result': {"status": "failed", "error": "Missing context data"},
                        'tonality_messages': [],
                        'tonality_result': f"[Content generation failed - Missing context data]"
                    }
                    continue
                ...
                logger.info(f"✅ Completed extraction and tonality for '{subsection_name}'")
